 ...
 *
 * Three facts a BIOBUZZ driver needs and cannot infer:
 ...
 *    discs carry no letters or digits (owner ruling 2026-09-12); the row's accessible name says
 *    the same thing in words. It reuses DECODE's `.hopper` / `.hopper-pip` anatomy, so the empty
 *    slot's contrast-audited ring is the same one.
 ...
 *
 * Rendered INSIDE the existing row, so it inherits the chip styles and the touch-layout
 * suppression. ALLIANCE-level facts are the score bar's, not this row's — see `BiobuzzScoreBar`.
 */
export function BiobuzzHudChips({ hud }: GameHudProps) {
  const r = sliceOf(hud)?.robot;
  if (!r) return null;
  const held = r.held ?? [];
  const free = Math.max(0, r.cap - held.length);
